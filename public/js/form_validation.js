    ...
    window.addEventListener("message", function (event) {
        ...
        if (event.data.hasOwnProperty("type")) {
            if (event.data.type == "css") {
                var extraHeight = 24;
                if (event.data.element_id !== "contact_us" && 
                    event.data.element_id !== "trade_form_with_autodropdown" && 
                    event.data.element_id !== "trade_form_with_vin" &&
                    event.data.element_id !== "document_upload_form" &&
                    event.data.element_id !== "license_form") {
                    extraHeight = 180;
                }
                $(`#${event.data.element_id}`).css("min-height", parseInt(event.data.value) + extraHeight);
            }
        }
